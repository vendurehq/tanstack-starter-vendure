...
export function AddressesClient({ addresses, countries }: AddressesClientProps) {
    ...
    const createCustomerAddress = useServerFn(createAddress);
    const updateCustomerAddress = useServerFn(updateAddress);
    const removeCustomerAddress = useServerFn(deleteAddress);
    const makeDefaultShipping = useServerFn(setDefaultShippingAddress);
    const makeDefaultBilling = useServerFn(setDefaultBillingAddress);
    ...
    const handleSetDefaultShipping = async (addressId: string) => {
        ...
        try {
            await makeDefaultShipping({data: {id: addressId}});
            router.refresh();
        } catch (error) {
    ...
    const handleSetDefaultBilling = async (addressId: string) => {
        ...
        try {
            await makeDefaultBilling({data: {id: addressId}});
            router.refresh();
        } catch (error) {
    ...
    const confirmDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            await removeCustomerAddress({data: {id: addressToDelete}});
            router.refresh();
